...
class OperationQueue: NSOperationQueue {
    ...
    override func addOperation(operation: NSOperation) {
        if let op = operation as? Operation {
            ...
            let dependencies = op.conditions.map {
            ...
            */
//            let concurrencyCategories: [String] = op.conditions.filter({ (condition) -> Bool in
//              
//              if !condition.dynamicType.isMutuallyExclusive { return false } else {return true}
//            }). map { condition in
...
//            }
  ...
  override func addOperations(ops: [AnyObject], waitUntilFinished wait: Bool) {
    
    /*
    The base implementation of this method does not call `addOperation()`,
    so we'll call it ourselves.
    */
    for operation in ops as! [Operation] {
