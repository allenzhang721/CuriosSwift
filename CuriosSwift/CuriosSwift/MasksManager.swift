...
struct MaskAttribute {
  ...
  func sizeToFit(size: CGSize) -> CGSize {
    if fixed {
      ...
      let fitWidth = width * minScale
      let fitHeight = height * minScale
...
class MasksManager {
  ...
  static var maskTypes: [String: MaskAttribute] {
    ...
    return [
      ...
      "ANone": MaskAttribute(width: 100, height: 100, fixed: false)
  ...
  class func imageOfMask(type: String, AtFrame frame: CGRect, completed:(UIImage?, Bool) -> ()) -> NSOperation {
    ...
    operation.addExecutionBlock { [unowned operation] () -> Void in
      ...
      let maskAttribute = self.maskTypes[type]
      let fitSize = maskAttribute?.sizeToFit(frame.size)
      let fr = CGRect(origin: frame.origin, size: fitSize!)
      if let image = self.drawMaskImage(type, AtFrame: fr) {
        dispatch_async(dispatch_get_main_queue(), { () -> Void in
  ...
  class func bezierPathWithMask(type: String, atFrame frame: CGRect) -> UIBezierPath {
    ...
    default:
      return CuriosMaskKit.drawNonePath(frame: frame)
    }
  } // end
  ...
  private class func drawMaskImage(type: String, AtFrame frame: CGRect) -> UIImage? {
    ...
    case "ANone":
      return CuriosMaskKit.imageOfANone(frame: frame)
    default:
      return nil
    }
