...
class  CountryCodeHelper {
  ...
  static let defaultSupportZones: [[String: String]] = {
    
    let path = NSBundle.mainBundle().pathForResource("SupportCountryZone", ofType: nil)!
    let data = NSData(contentsOfFile: path)
    let result = NSJSONSerialization.JSONObjectWithData(data!, options: NSJSONReadingOptions(0), error: nil) as! [[String: String]]
    return result
  }()
  ...
  */
  ...
  class func defaultZones() -> [Zone] {
    
    let supportzones = CountryCodeHelper.defaultSupportZones
    
    let zones = supportzones.map({ (zoneDic) -> Zone in
      let zoneCode = zoneDic["zone"]!
      let rule = zoneDic["rule"]!
